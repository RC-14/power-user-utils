 ...
 */
export const qs = <T extends Element>(selector: string, parent: Document | Element = document) =>
 ...
 */
export const qsa = <T extends Element>(selector: string, parent: Document | Element = document) =>
...
export const sendRuntimeMessage = (
  target: Exclude<RuntimeMessage['target'], 'content'>,
  fragmentId: RuntimeMessage['fragmentId'],
...
type showMessageOptions = {
  ...
  /** The CSS size of the message */ size?:
    | 'xx-small'
    | 'x-small'
    | 'smaller'
    | 'small'
...
export const showMessage = (
  message: string,
  { color = 'darkgrey', duration = 3_000, size = 'large' }: showMessageOptions = {}
) => {
  // Get the wrapper element
  let wrapper = qs<HTMLDivElement>('div#webext-messages');
  if (wrapper === null) {
    // If the wrapper element doesn't exist, create it
    wrapper = document.createElement('div');

    wrapper.id = 'webext-messages';
 ...
 */
export const generateIDBGetter = (
  name: string,
  version: number,
  upgradeneededListener: (event: IDBVersionChangeEvent & Record<'target', IDBOpenDBRequest>) => unknown,
  blockedListener?: (event: Event) => unknown
) => {
  return () =>
    new Promise<IDBDatabase>((resolve, reject) => {
      ...
      request.addEventListener('error', (event) =>
        reject(new Error(`Couldn't get iDB (name: "${name}", version: ${version}): ${request.error}`))
      );
      ...
      request.addEventListener(
        'upgradeneeded',
        upgradeneededListener as (event: IDBVersionChangeEvent) => ReturnType<typeof upgradeneededListener>
      );
    });
};
 ...
 * If it's an HTML element id and classes get added to the back so that it's a valid CSS selector.
 */
export const stringifyNode = (node: Node) => {
  if (!(node instanceof HTMLElement)) return node.nodeName;

  let nodeString = node.nodeName;
  if (node.id) nodeString += '#' + node.id;
  if (node.classList.length) nodeString += '.' + [...node.classList.values()].join('.');
  return nodeString;
};

/**
 * Reconstructs the path to the given node.
 *
 * @param node
 * @param backtraceLimit Optional: Stops path reconstruction when encountering this element.
 * @returns The path to the given node as a CSS selector including ids and classes.
 */
export const getNodePath = (node: Node, backtraceLimit?: Element) => {
  let nodePath = stringifyNode(node);
  let currentNode = node.parentElement;
  while (currentNode && currentNode !== backtraceLimit) {
    nodePath = stringifyNode(currentNode) + ' > ' + nodePath;
